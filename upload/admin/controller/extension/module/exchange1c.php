...
class ControllerExtensionModuleExchange1c extends Controller
{
	...
	public function index($refresh = false)
	{
		...
		$select_attribute_group_name_mode = array(
			...
			'divider'			=> $this->language->get('text_text_in_divider')
		);
		...
		$params = array(
			...
			'product_stock_status_pre_order'					=> array('type'	=> 'select', 'options' => $select_stock_statuses),
			...
			'product_special_group_id'					=> array('type' => 'input'),
		);
	...
	public function downloadOrders()
	{
		...
		if ($this->config->get('exchange1c_convert_orders_cp1251') == 1) {
			...
			// Перевіряємо, чи текст у форматі UTF-8
			if (!empty($orders) && mb_detect_encoding($orders, 'UTF-8', true)) {
				// Конвертуємо текст із UTF-8 у CP1251
				$orders = iconv('UTF-8', 'CP1251//TRANSLIT', $orders);

				// Замінюємо кодування в тексті (наприклад, у мета-тегах)
				...
				// Лог чи повідомлення про помилку
				error_log('Неможливо конвертувати: текст не у форматі UTF-8 або порожній.');
			}
		}
