...
class ModelExtensionExchange1c extends Model
{
	...
	public function cleanDB()
	{
		...
		// Удаляем связи со складами
		$this->log("Очистка связей со складами...", 2);
		$this->query('TRUNCATE TABLE `' . DB_PREFIX . 'product_to_multistore`');
		$result .=  "Связи со складами\n";
	...
	private function seoGenerateProduct($product_id, $data)
	{
		...
		$tags = array(
			...
			'{model}'		=> isset($data['model']) ? str_replace(' ', '', $data['model']) : '',
	...
	private function prepareQueryProduct($data, $mode = 'set')
	{
		...
		$sql = array();
		if (isset($data['model']))
			$sql[] = $mode == 'set'		? "`model` = '" .				$this->db->escape(str_replace(' ', '', $data['model'])) . "'"				: "`model`";
		if (isset($data['sku']))
	...
	private function addProduct(&$data)
	{
		...
		if (isset($data['reward_points'])) {
			$this->setProductReward($product_id, $data);
		}
	...
	private function updateProduct($product_id, $data)
	{
		...
		if ($old_data['quantity']) {
			...
			if ($data['pre_order'] == 1) {
				$data['stock_status_id'] = (int)$this->config->get('exchange1c_product_stock_status_pre_order');
			} else {
				if ((int)$this->config->get('exchange1c_product_stock_status_off')) {
					$data['stock_status_id'] = (int)$this->config->get('exchange1c_product_stock_status_off');
				}
	...
	private function setProductPrice($table_price, $price, $product_id, $customer_group_id, $old_prices)
	{
		...
		if ($table_price == 'discount') {
			...
			foreach ($old_prices['special'] as $old_price) {
				if ($old_price['customer_group_id'] == $customer_group_id && $old_price['product_special_group_id'] == $this->config->get('exchange1c_product_special_group_id')) {
					// if ($old_price['price'] != $price) {
						$this->query("UPDATE `" . DB_PREFIX . "product_special` SET `price` = '" . $data_price['price'] . "' WHERE `product_special_id` = '" . $old_price['product_special_id'] . "' AND `product_special_group_id` = '".$this->config->get('exchange1c_product_special_group_id')."'");
					// }
					return;
				}
			}
			...
			$this->query("INSERT INTO `" . DB_PREFIX . "product_special` SET `product_id` = " . (int)$product_id . ", `priority` = " . (int)$data_price['priority'] . ", `customer_group_id` = " . (int)$data_price['customer_group_id'] . ", `price` = '" . (float)$data_price['price'] . "', `date_start` = '" . $this->NOW . "', `product_special_group_id` = '".$this->config->get('exchange1c_product_special_group_id')."'");
		}
	} // setProductPrice()
	...
	private function getProductPrices($product_id)
	{
		...
		$query = $this->query("SELECT * FROM `" . DB_PREFIX . "product_special` WHERE `product_id` = " . $product_id . " AND `product_special_group_id` = '".$this->config->get('exchange1c_product_special_group_id')."'");
		...
		foreach ($query->rows as $special) {
			$data_price['special'][$special['product_special_id']] = array(
				...
				'product_special_group_id'	=> $special['product_special_group_id'],
	...
	private function updateOffers($product_id, $data, $old_data)
	{
		...
		$multistores = $this->model_extension_module_multistore->getMultistores(['filter_status' => 1]);

		$this->query("DELETE FROM `" . DB_PREFIX . "product_to_multistore` WHERE `product_id` = " . (int)$product_id);

		foreach ($multistores as $mstore) {

			$qty = 0;

			if (!empty($data['multistore'][$mstore['multistore_id']])) {
				$qty = $data['multistore'][$mstore['multistore_id']];
			}

			$this->query("INSERT INTO `" . DB_PREFIX . "product_to_multistore` SET `product_id` = " . (int)$product_id . ", `multistore_id` = " . (int)$mstore['multistore_id'] . ", `quantity` = " . floatval($qty));
		}

		// if (isset($data['multistore'])) {
		// 	foreach ($data['multistore'] as $multistore_id => $qty) {
		// 		$query = $this->query("SELECT * FROM `" . DB_PREFIX . "product_to_multistore` WHERE `product_id` = " . (int)$product_id . " AND  `multistore_id` = " . (int)$multistore_id);

		// 		if ($query->num_rows) {
		// 			$this->query("UPDATE `" . DB_PREFIX . "product_to_multistore` SET `quantity` = " . floatval($qty) . " WHERE `product_id` = " . (int)$product_id . " AND  `multistore_id` = " . (int)$multistore_id);
		// 		} else {
		// 			$this->query("INSERT INTO `" . DB_PREFIX . "product_to_multistore` SET `product_id` = " . (int)$product_id . ", `multistore_id` = " . (int)$multistore_id . ", `quantity` = " . floatval($qty));
		// 		}
		// 	}
		// }
	...
	private function setProduct(&$data)
	{
		...
		if (!$product_id) {
			...
			if ($this->config->get('exchange1c_product_sync_mode') == 'sku') {
				...
				$this->log("Поиск товара по модели: " . str_replace(' ', '', $data['model']), 2);
				if (empty($data['model'])) {
					$this->log("ВНИМАНИЕ! пустое поле Модель! Товар пропущен. Проверьте товар " . $data['name'], 2);
					return false;
				}
				$product_id = $this->getProductByModel(str_replace(' ', '', $data['model']));
				if ($product_id)
					$check_link = true;
	...
	//TODO якщо буде декілька груп покупців внести зміни
	/**
	 * Set reward points for product
	 * @param int $product_id
	 * @param array $data
	 * @return void
	 */
	public function setProductReward($product_id, $data)
	{
		$this->query("DELETE FROM `" . DB_PREFIX . "product_reward` WHERE `product_id` = " . (int)$product_id);
		$this->query("INSERT INTO `" . DB_PREFIX . "product_reward` SET `product_id` = '" . (int)$product_id . "', `points` = '" . (int)$data['reward_points'] . "', `customer_group_id` = '3', `type` = '" . $this->db->escape($data['reward_type']) . "'");
	}
	...
	private function parseRequisite($xml, &$data)
	{
		...
		foreach ($xml->ЗначениеРеквизита as $requisite) {
			...
			if (isset($product_model_in_requisite_tag)) {
				...
				$this->log("> Реквизит: " . $name . " со значением " . str_replace(' ', '', $data['model']) . " записан в КОД товара", 2);
			} else {
				switch ($name) {
					case 'Вес':
	...
	private function parseClassifierAttributes($xml)
	{
		...
		foreach ($properties as $property) {
			...
			if ($this->config->get('exchange1c_attribute_group_name_mode') == 'divider') {
				$split = explode(';', $data['name']);

				if (count($split) > 1) {
					$data['name'] = $split[1];
					$group_name 	= $split[0];
					$this->log("Название группы : " . $group_name, 2);
				} else {
					$data['name'] = $split[0];
				}
			}
	...
	private function parseProducts($xml)
	{
		...
		foreach ($xml->Товар as $num => $product) {
			...
			// Модель
			// $data['model'] = $product->КодТовараНаСайте ? htmlspecialchars(trim((string)$product->КодТовараНаСайте)) : "";
			...
			//МОДЕЛЬ
			if ($product->КодТовараНаСайте && $this->config->get('exchange1c_product_model_in_requisite') != 1) {
				$data['model'] = htmlspecialchars(preg_replace('/\D/', '', $product->КодТовараНаСайте));
			...
			// ГРУППЫ ТОВАРОВ
			if ($product->КатегорииСайта && $this->config->get('exchange1c_product_category_no_import') != 1) {
				// Если надо обновлять категории товара
				$data['categories']	= $this->parseProductCategories($product->КатегорииСайта);
			...
			// ТОВАРНАЯ КАТЕГОРИЯ
			$data['product_category'] = ($product->КатегорияТовара) ? (string)$product->КатегорияТовара : "";
	...
	private function parseMultistoreQuantity($xml)
	{
		$data = array();

		if ($xml->КоличествоНаСкладе) {
			foreach ($xml->КоличествоНаСкладе as $stock) {
				$guid = ((string)$stock->ИдСклада);

				$query = $this->query("SELECT `multistore_id` FROM `" . DB_PREFIX . "multistore` WHERE `guid` = '" . $this->db->escape($guid) . "'");

				if ($query->num_rows) {
					$data[$query->row['multistore_id']] = (int)$stock->Количество;
				}
			}
		}
	...
	private function parsePrices($xml, $product_id)
	{
		...
		foreach ($config_prices as $config_price_type) {
			...
			foreach ($xml->Цена as $price_data) {
				...
				$price	= $price_data->ЦенаЗаЕдиницу ? (int)str_replace(',', '.', $price_data->ЦенаЗаЕдиницу) : 0;
				...
				if (
					...
					if ($config_price_type['table_price'] == 'discount') {
						...
						$this->query("INSERT INTO `" . DB_PREFIX . "product_special` SET `product_id` = " . (int)$product_id . ", `priority` = " . (int)$config_price_type['priority'] . ", `customer_group_id` = " . (int)$config_price_type['customer_group_id'] . ", `price` = '" . (float)$price . "', `date_start` = '" . $this->NOW . "', `product_special_group_id` = '".$this->config->get('exchange1c_product_special_group_id')."'");
					}
				}
			} // foreach ($xml->Цена as $price_data)
	...
	private function parseOffers($xml)
	{
		...
		foreach ($xml->Предложение as $offer) {
			...
			// СКЛАДЫ
			if ($offer->КоличествоНаСкладах) {
				$data['multistore'] = $this->parseMultistoreQuantity($offer->КоличествоНаСкладах);
			}
			...
			//БОНУСИ
			if ($offer->ПроцентБонуса) {
				$data['reward_points'] = (float)$offer->ПроцентБонуса;
				$data['reward_type'] = 'percent';
				$this->log("reward points прочитано", 2);
			} elseif ($offer->Бонус) {
				$data['reward_points'] = (float)$offer->Бонус;
				$data['reward_type'] = 'number';
				$this->log("reward points прочитано", 2);
			}

			if ($offer->ВозможенПредзаказ == 1) {
				$data['pre_order'] = 1;
			} else {
				$data['pre_order'] = 0;
			}
	...
	private function setCustomer($order)
	{
		...
		$order['lastname'] = !empty($order['lastname']) ? $order['lastname'] : $order['firstname'];
	...
	public function queryOrdersExport()
	{
		...
		if ($this->config->get('exchange1c_orders_export_modify')) {
			...
			$query = $this->query("SELECT `order_id`,`order_status_id` FROM `" . DB_PREFIX . "order` WHERE `date_modified` BETWEEN STR_TO_DATE('" . $from_date . "', '%Y-%m-%d %H:%i:%s') AND STR_TO_DATE('" . $to_date . "', '%Y-%m-%d %H:%i:%s') AND `order_status_id` != 0 ORDER BY `date_modified` DESC");
	...
	//TODO-ARI формування вигрузки замовлення
	/**
	 * ver 14
	 * update 2018-10-05
		...
		if (count($orders_export)) {
			...
			foreach ($orders_export as $order_id => $order_status_id) {
				...
				foreach ($products as $product) {
					...
					$document_products['Товар' . $product_counter] = array(
						'Ид'             => $product_guid,
					...
					$current_product = &$document_products['Товар' . $product_counter];
				...
				$document_counterparty = ['Контрагент' => $this->setCustomer($order)];
				if ($this->ERROR) continue;

				// РЕКВИЗИТЫ ДОКУМЕНТА
				$document_requisites = $this->setDocumentRequisites($order, '');
				if ($this->ERROR) continue;

				//TODO-ARI ОПЛАТА
				$document_payment = $this->setPayment($order);
				if ($this->ERROR) continue;

				//TODO-ARI ДОСТАВКА
				$document_delivery = $this->setDelivery($order);
				if ($this->ERROR) continue;
				...
				$document['Документ' . $document_counter] = [
					...
					'Контрагенты' => $document_counterparty,
					'ЗначенияРеквизитов' => $document_requisites,
					'Товары'      => $document_products,
					'Оплата'      => $document_payment,
					'Доставка'    => $document_delivery,
	...
	//TODO-ARI	ОПЛАТА метод
	private function setPayment($order)
	{
		$document_payment = array();
		...
		$current_payment = &$document_payment;

		if (!empty($order['ukrcredits_order_id'])) {
			$current_payment['Кредит'] 	= [
				'Ид' => $order['ukrcredits_order_id'],
				'Статус' => $order['ukrcredits_order_status'],
				'Сумма' => $order['total'],
			];
	...
	//TODO-ARI	ДОСТАВКА метод
	private function setDelivery($order)
	{
		$this->load->model('extension/module/simplecustom');
		$this->load->model('extension/module/multistore');

		$customInfo = $this->model_extension_module_simplecustom->getCustomFields('order', $order['order_id'], $order['language_code']);
		...
		switch ($order['shipping_code']) {
			case 'novaposhta.poshtomat':

				$devivery_address = $order['payment_city'] .', '. $customInfo['shipping_poshtomat_address_1'];
				$delivery_type = 'Нова пошта';
				$devivery_method = 'Поштомат';
				$devivery_code = $order['shipping_code'];
				$uuid = $this->getUuidDepartmentByTitle($customInfo['shipping_poshtomat_address_1']);

				break;
			case 'novaposhta.department':

				$devivery_address = $order['payment_city'] .', '. (!empty($customInfo['shipping_departmans_np']) ? $customInfo['shipping_departmans_np'] : '');
				$delivery_type = 'Нова пошта';
				$devivery_method = 'Відділення';
				$devivery_code = $order['shipping_code'];
				$uuid = $this->getUuidDepartmentByTitle($customInfo['shipping_departmans_np']);

				break;
			case 'novaposhta.doors':
				$devivery_address = $order['payment_city'] . ', ' . $customInfo['shipping_street'] . ', ' . $customInfo['shipping_house'] . ', ' . $customInfo['shipping_flat'];
				$delivery_type = 'Нова пошта';
				$devivery_method = 'Адресна доставка';
				$devivery_code = $order['shipping_code'];
				break;
			default:

				$multistore = $this->model_extension_module_multistore->getMultistore(str_replace('multistore_pickup.multistore_', '', $order['shipping_code']));
				$devivery_address = $multistore['name'];
				$delivery_type = 'Самовивіз';
				$devivery_method = $multistore['type'];
				$uuid = $multistore['guid'];
				$devivery_code = $order['shipping_code'];
				break;
		}

		$document_delivery = array();

		$document_delivery = [
			'ВидДоставки' => $delivery_type,
			'МетодДоставки' => $devivery_method,
			'UUID' => $uuid,
			'Код' => $devivery_code,
			'Адрес' => $devivery_address,
		];

		return $document_delivery;
	}

	public function getUuidDepartmentByTitle($title)
	{
		$query = $this->query("SELECT * FROM `" . DB_PREFIX . "novaposhta_departments` WHERE `Description` = '" . $title . "'");
		if ($query->num_rows) {
			return $query->row['Ref'];
		}
		return false;
	}

	public function getUuidMultistoreById($id)
	{
		$query = $this->query("SELECT * FROM `" . DB_PREFIX . "multistore` WHERE `multistore_id` = '" . $id . "'");
		if ($query->num_rows) {
			return $query->row['guid'];
		}
		return false;
	...
	private function parseDocumentProducts($xml, &$doc)
	{
		...
		foreach ($xml->Товар as $product) {
			...
			if ($product->Артикул) {
				...
				$data['model'] = str_replace(' ', '', $product->Артикул);
			}
			...
			if ($product->ПроцентБонуса) {
				$data['reward_points'] = (float)$product->ПроцентБонуса;
				$data['reward_type'] = 'percent';
				$this->log("reward points прочитано", 2);
			} elseif ($product->Бонус) {
				$data['reward_points'] = (float)$product->Бонус;
				$data['reward_type'] = 'number';
				$this->log("reward points прочитано", 2);
			}

			if ($product->ВозможенПредзаказ) {
				$data['pre_order'] = 1;
			} else {
				$data['pre_order'] = 0;
			}
	...
	private function parseClassifierCategories($xml, $parent_id = 0, &$num_categories)
	{
		...
		foreach ($xml->КатегорияСайта as $xml_category) {
			...
			if ($xml_category->Ид && $xml_category->Наименование) {
				...
				if ($category_id) {
					...
					if ($this->config->get('exchange1c_category_new_no_create') == 1) {
						...
						if ($xml_category->КатегорииСайта) {
							$this->parseClassifierCategories($xml_category->КатегорииСайта, $category_id, $num_categories);
							if ($this->ERROR) return false;
						}
						continue;
					} else {
	...
	private function parseClassifier($xml)
	{
		...
		if ($xml->КатегорииСайта && $this->config->get('exchange1c_categories_no_import') != 1) {
			...
			$num_categories = 0;
			$this->parseClassifierCategories($xml->КатегорииСайта, 0, $num_categories);
